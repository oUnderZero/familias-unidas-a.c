...
export const AdminDashboard: React.FC = () => {
  ...
  // Modal View Mode
  const [modalMode, setModalMode] = useState<"SIMPLE_QR" | "DESIGNER">(
    "SIMPLE_QR"
  );
  ...
  const handleShowQr = (member: Member) => {
    ...
    if (!active) {
      alert(
        "Este miembro no tiene una credencial activa. Por favor edite el miembro y emita una nueva credencial."
      );
      return;
    }
  ...
  const filteredMembers = members.filter((m) => {
    const matchesSearch =
      ...
      (m.colony || "").toLowerCase().includes(searchTerm.toLowerCase()); // Aquí el cambio más importante
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Gestión centralizada de credenciales y accesos.
            </p>
          </div>
        </div>
        <Link
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredMembers.map((member) => {
                ...
                const expired = activeCred
                  ? isExpired(activeCred.expirationDate)
                  : true;
                  ...
                  >
                        ...
                        <span>
                          {member.street ? (
                            <>
                              ...
                              {member.colony && (
                                <span className="block text-xs text-sky-600 font-medium">
                                  {member.colony}
                                </span>
                              )}
                            </>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-1">
                        {!activeCred ? (
                          <span className="text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded w-fit">
                            Sin credencial
                          </span>
                        ) : (
                          <>
                            <span
                              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold w-fit border ${
                      ...
                      <div className="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                        {activeCred && (
                          <>
                            <button
                              onClick={() =>
                                handlePreview(member.id, activeCred.token)
                              }
                              title="Vista Previa"
      ...
      {isQrModalOpen && selectedMember && selectedCredential && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 print:p-0 print:bg-white print:absolute overflow-y-auto">
          <div
            className={`bg-white rounded-xl p-8 shadow-2xl flex flex-col items-center w-full print:shadow-none print:w-full print:max-w-none relative my-auto ${
              modalMode === "DESIGNER" ? "max-w-4xl" : "max-w-sm"
            }`}
          >
            <button
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-4 print:hidden">
              Credencial Digital
            </h3>

            {/* Tabs */}
            {(() => {
              const designerDisabled =
                selectedCredential.status !== "ACTIVE" ||
                isExpired(selectedCredential.expirationDate);
              return (
                <div className="flex gap-2 mb-6 bg-slate-100 p-1 rounded-lg w-full print:hidden">
                  ...
                  <button
                    ...
                    title={
                      designerDisabled
                        ? "Disponible solo con credencial vigente"
                        : "Abrir Diseñador"
                    }
                  >
            ...
            {modalMode === "SIMPLE_QR" && (
              <>
                <div className="border-4 border-slate-900 p-4 rounded-xl bg-white text-center mt-2">
                  <div className="mb-2 font-bold text-slate-900 uppercase tracking-wider text-sm">
                ...
                <div className="mt-6 text-center print:hidden">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Vence: {selectedCredential.expirationDate}
                  </p>
                  <div className="flex justify-center gap-2 mt-4">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-slate-900 text-white rounded text-sm hover:bg-slate-800 flex items-center gap-2"
                    >
